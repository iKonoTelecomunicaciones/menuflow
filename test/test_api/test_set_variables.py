...
# ---------- Custom scopes (scope == None) ------------------------------


@pytest.mark.asyncio
async def test_custom_scope_reserved_scopes(mock_room):
    """scope None: route/room/node values produce a set_variable each."""
    resp = await set_variables(
        make_mock_request(
            {
                "scope": None,
...
async def test_custom_scope_existing_custom_scope_no_new_scope_log(mock_room, caplog):
    ...
    await set_variables(
        make_mock_request({"scope": None, "variables": {"catalog": {"sku": "X1"}}})
    )

    mock_room.set_variable.assert_awaited_once_with(variable_id="sku", value="X1", scope="catalog")
    assert not any("Detecting new scope (catalog)" in r.message for r in caplog.records)
...
async def test_custom_scope_new_scope_emits_info_log(mock_room, caplog):
    ...
    await set_variables(
        make_mock_request({"scope": None, "variables": {"billing": {"invoice_id": 99}}})
    )

    mock_room.set_variable.assert_awaited_once_with(
        variable_id="invoice_id", value=99, scope="billing"
    )
    assert any("Detecting new scope (billing)" in r.message for r in caplog.records)
